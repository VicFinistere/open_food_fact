...
def admin_interface(url="https://fr.openfoodfacts.org/"):
    """
    Administrator menu
...
def create_tables():
    ...
    with warnings.catch_warnings():
        ...
        create_table_categories_req = "CREATE TABLE IF NOT EXISTS categories(" \
                                      "id_category INTEGER PRIMARY KEY AUTO_INCREMENT UNIQUE, name TEXT) "

        create_table_products_req = "CREATE TABLE IF NOT EXISTS products( " \
                                    "id INTEGER PRIMARY KEY AUTO_INCREMENT UNIQUE, " \
                                    "name TEXT, grade VARCHAR(50), url TEXT, id_category INTEGER(1), " \
                                    "FOREIGN KEY fk_category(id_category) REFERENCES categories(id_category) " \
                                    "ON UPDATE CASCADE ON DELETE RESTRICT) "

        cursor.execute(create_table_categories_req)
        cursor.execute(create_table_products_req)
        cursor.close()
        connection.close()
...
def calculate_parsing_loops(product_count):
    ...
    # if products > 20 : page range is products // 20
        ...
        if amount_last_page_products != 0:
            ...
            # If the amount has two numbers ( over 10 )
            if len(listed_amount) > 1:

                # If the first number is odd
                # because it goes 20 by 20
                if listed_amount[-2] % 2 != 0:

                    # We want to get 1. products in the last page
                    # ( 16 for example )
